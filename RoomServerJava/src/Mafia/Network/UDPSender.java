...
public class UDPSender extends Sender{
    ...
    public void run(){
        if(initialized){
            while (running){
                ...
                MessageJ message = MessageParser.parse_message(packet.getData(), packet.getLength(), ip, port);
    ...
    private void process(int index){
        ...
        if(notFullMessages.get(index).is_complete()){
            ...
            try {
                ...
                MessageReceived messageReceived = new MessageReceived(whole, this);
    ...
    public void send_message(MessageJ message){
        ...
        for (int i = 0; i < count; i++){
            ...
            byte[] buffer = MessageParser.wrap_message(current);
